...
func main() {
	...
	e.GET("/ping", func(c echo.Context) error {
		// Ping the database first to ensure connectivity
		if err := db.Ping(); err != nil {
			fmt.Println("Ping Failed:", err)
			return c.HTML(http.StatusBadRequest, "Database Ping Failed!")
		}

		// Execute the query to insert a timestamp into the database
		_, err := db.Exec("INSERT INTO timestamp (timestamp) VALUES ($1)", time.Now())
		if err != nil {
			fmt.Println("Query Execution Failed:", err)
			return c.HTML(http.StatusInternalServerError, "Database Query Execution Failed!")
		}
